...
def main():
    ...
    for character in old_options["characters"]:
        new_options["characters"][character["name"]] = { "unique to": character["unique to"] }
    ...
    for class_info in old_options["classes"]:
        ...
        unique_to = class_info["unique to"]
        if len(incompatible) > 0:
            incompatible_reversed = [character for character in character_names if character not in incompatible]
            unique_to = class_info["unique to"] + incompatible_reversed if len(class_info["unique to"]) > 0 else incompatible_reversed
        new_options["classes"][class_info["name"]] = { "unique to": unique_to, "requirements": class_info["requirements"] }
